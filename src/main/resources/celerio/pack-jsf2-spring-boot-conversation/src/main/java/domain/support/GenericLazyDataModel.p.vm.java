...
$output.require("org.primefaces.model.FilterMeta")##
$output.require("org.primefaces.model.LazyDataModel")##
$output.require("org.primefaces.model.SortMeta")##
...
public abstract class ${output.currentClass}<E extends Identifiable<PK>, PK extends Serializable, F extends GenericSearchForm<E, PK, F>> extends LazyDataModel<E> {
	...
	public List<E> load(int first, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {
      ...
      SearchParameters sp = populateSearchParameters(first, pageSize, sortBy, filterBy);
    ...
    */
    protected SearchParameters populateSearchParameters(int first, int pageSize, Map<String, SortMeta> sortBy, Map<String, FilterMeta> filterBy) {
        SearchParameters sp = searchForm.toSearchParameters();
        sp.setFirst(bypassFirstOffset ? 0 : first);
        bypassFirstOffset = false;
        sp.setPageSize(pageSize);
        
        sortBy.entrySet().stream().forEach(e->sp.orderBy( convert(e.getValue().getOrder()),e.getKey(), repository.getType()));
